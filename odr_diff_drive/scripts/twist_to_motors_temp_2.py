...
#turn F_left_start
        ...
        #  self.right=0
            ...
            self.targetLWV=self.left
            self.targetRWV=self.right

            self.RWV_range=self.targetRWV-self.old_RWV
            self.LWL_range=self.targetLWV-self.old_LWV
            self.LWV_Scale=self.LWV_range/20
            self.RWV_Scale=self.RWV_range/20

            if not (self.targetLWV==0 and self.targetRWV==0 ):
                for i in range (1,20):
                    self.wheel_vel.Vl.data=self.old_LWV+(i*self.LWV_Scale)
                    self.wheel_vel.Vr.data=self.old_RWV+ (i*self.RWV_Scale)
